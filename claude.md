...
An MCP (Model Context Protocol) server that exposes sales call transcripts to Claude for AI-powered analysis.

---

## Project Overview

This project demonstrates how to build an MCP server that bridges local data (sales call transcripts) with Claude, enabling natural language queries like:

- "What calls do we have with telecom companies?"
- "Search for mentions of procurement delays"
- "Summarize the key themes from the TeleCom Nexus interview"
...
│    (47 markdown files with metadata)     │
└─────────────────────────────────────────┘
```
...
|------|---------|------------|
| `list_calls` | List/filter transcripts | `company`, `deal_stage`, `limit` |
| `get_call` | Get full transcript | `call_id` |
| `search_calls` | Keyword search | `query`, `limit` |
...
│   ├── transcript_loader.py # Markdown + YAML parsing
│   └── search.py           # Keyword search with context
├── transcripts/            # 47 call transcripts
...
- **Transcript Loader**: Handles both YAML frontmatter and markdown metadata tables
- **Search**: Returns ~75 words of context around matches with speaker/timestamp extraction
- **Caching**: Transcripts loaded once at startup for sub-second response times
- **Error Handling**: Graceful fallbacks for malformed files, clear error messages
